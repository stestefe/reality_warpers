...
public class VRCooldownVisualizer : MonoBehaviour
{
    [Header("References")]
    public Transform controllerTransform;
    public ClientBlackscreenTrigger triggerScript;
    ...
    void Start()
    {
        ...
        {
            triggerScript = GetComponent<ClientBlackscreenTrigger>();
            if (triggerScript == null)
            {
                Debug.LogError("VRCooldownVisualizer: ClientBlackscreenTrigger not found! Please assign it in the inspector.");
                enabled = false;
                return;
            }
        }
    ...
    void Update()
    {
        if (triggerScript == null || radialIndicator == null) return;

        float cooldownProgress = triggerScript.GetCooldownProgress();
        
        UpdateRadialMesh(1f - cooldownProgress);

        if (indicatorMaterial != null)
        {
            Color targetColor = cooldownProgress > 0 ? cooldownColor : readyColor;
            indicatorMaterial.color = targetColor;
        }

        if (enablePulse && cooldownProgress <= 0)
        {
